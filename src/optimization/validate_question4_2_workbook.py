"""Read-only, independent reconciliation of result4-2.xlsx against the Q4-2
LDR dispatch ledger, with the fluctuating-price bill recomputed from
attachment 4 (plan sum p_act*g, emergency 5*p_act*b)."""
import argparse
import json
...
def main():
    ...
    parser.add_argument("--output-dir", type=Path, default=ROOT / "outputs/question4/result4-2")
    out = parser.parse_args().output_dir
    f = pd.read_csv(out / "question2_schedule.csv", parse_dates=["date"])
    w = openpyxl.load_workbook(out / "result4-2.xlsx", data_only=True)
    template = openpyxl.load_workbook(ROOT / "附件/附件5/result4-2.xlsx", data_only=False)
    ...
    for j, (date, day) in enumerate(f.groupby("date")):
        ...
        d_idx = (date - year0).days
        p_act = prices[d_idx]
        assert abs(np.dot(g, p_act) - row[146]) < 1e-5
    ...
    result = {"passed": True, "headers_preserved": True, "plan_days": 334,
              "plan_intervals": 48096, "battery_blocks": 2004, "emergency_rows": e.max_row - 1,
              "max_workbook_schedule_error_kwh": max_error,
              "independent_planned_bill": planned_bill,
              "independent_emergency_bill": emergency_bill,
              "independent_total_bill": planned_bill + emergency_bill}
    (out / "question4_2_workbook_validation.json").write_text(json.dumps(result, indent=2), encoding="utf-8")

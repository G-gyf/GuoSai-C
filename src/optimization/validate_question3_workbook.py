"""Read-only, independent reconciliation of the exported result3 workbook
against the M612 dispatch ledger.

Checks: the four official worksheets and headers are preserved; the plan
sheet carries q0 with the template slot mapping (its own 00:00-00:10 lives
in the previous row's last column); the adjusted sheet carries the final qA
and its EQ column equals the segment net settlement; battery blocks and
boundary SOC match the ledger; the emergency sheet reconstructs the full
emergency vector; the four cost components are recomputed independently.
"""
import argparse
import json
...
def main():
    ...
    parser.add_argument("--output-dir", type=Path, default=ROOT / "outputs/question3/current")
    out = parser.parse_args().output_dir
    f = pd.read_csv(out / "M612" / "question3_schedule.csv", parse_dates=["date"])
    w = openpyxl.load_workbook(out / "result3.xlsx", data_only=True)
    template = openpyxl.load_workbook(ROOT / "附件/附件5/result3.xlsx", data_only=False)
    ...
    prices = np.r_[price_raw[-1], price_raw[:-1]]  # internal interval-start order
    ...
    # independent four-component recomputation
    ...
    result = {
        ...
        "four_component_recheck": {"retained": retained, "down": down, "up": up, "emergency": emergency_bill},
    }
    (out / "question3_workbook_validation.json").write_text(

...
针对固定电价下负载和光伏逐日变化、计划购电全天锁定而供电缺口须按五倍电价紧急补购的问题，建立“风险修正日前计划—同规则情景校准—日内因果执行”的分层优化模型。首先，以最近七天同一时段均值预测负载与光伏，利用最近21个有效历史日的成对残差构造净负荷情景；依据单时段购电的成本权衡，固定80%分位数形成风险修正曲线，并通过线性规划确定计划购电量及参考库存。其次，将全天划分为四个六小时阶段，以阶段开始前的累计预测误差均值为反馈信息，构造截断仿射库存保留阈值，采用有限预算差分进化算法每日校准七个参数。日前情景评价与日内运行共用充放电规则，在不改变当天购电计划的条件下，保证设备可行性和信息非预见性。最后，从2025年1月1日6000 kWh库存出发连续仿真，得到2—12月实际总购电费15,819,068.62元，其中计划费15,066,447.00元、紧急费752,621.62元；相比分位数固定保留基线和即时补缺基线分别节费203,482.07元和96,865.72元。逐时供需平衡、库存递推、跨日连续及结果表回读检查均通过。该模型在本年度已比较方案中表现出较低费用，采用其作为问题二主方案；不主张完整随机调度问题的全局最优性。
...
2. 每天0:00确定全部普通购电计划，日内不调整，普通购电按计划全额计费；未利用电量不退款、不向外网出售。
...
$$
\widehat L_{d,t}=\frac17\sum_{i=1}^7L_{d-i,t},\qquad
\widehat V_{d,t}=\frac17\sum_{i=1}^7V_{d-i,t},\qquad
\widehat N_{d,t}=\widehat L_{d,t}-\widehat V_{d,t}.
\tag{2}
$$

该方法利用近期同一时段的日内结构，参数少、可逐日更新。采用它的理由是结构清晰和信息可用性，并不假设七天均值是所有可能预测方法中的最优模型。
...
其中$[x]^+=\max(x,0)$。对负载和光伏分别截断可避免生成负功率；按完整日路径取样，保留样本中的日内相关性及同日负载—光伏配对关系。历史预测必须只用其自身此前七天的数据，不能用决策日重新拟合的残差替换历史样本外残差。
...
上述配置固定用于此次全年运行，不能因真实当天或全年费用重新选择当天参数。当前实现只给出预算内找到的候选，不证明七维经验目标全局最优。在337个完整残差窗日期中，335天达到最大迭代预算，2天满足算法终止条件；达到预算后仍可接受物理可行且满足基线检查的候选。不能将“被接受”写成“全局收敛”。

## 7 初始化、连续仿真与费用结算

### 7.1 初始化

1月1日0:00实际库存为6000 kWh；当天采用零计划冷启动，储能补缺后再紧急购电。该规则仅用于无历史数据的初始化，不作为经济最优策略。1月2—7日使用已有完整历史日的均值；1月8日起具有完整七天预测；只将完整七天预测产生的残差纳入正式残差池。1月9—28日逐渐积累残差，1月29日起可用完整21条。
...
1. 计算七天点预测与过去有效残差，构造21条情景和80%风险曲线。
...
| 指标 | 数值 | 单位 |
|---|---|---|
| 计划购电量 | 23,973,797.15 | kWh |
| 计划购电费 | 15,066,447.00 | 元 |
| 紧急购电量 | 146,441.68 | kWh |
| 紧急购电费 | 752,621.62 | 元 |
| 实际总费 | 15,819,068.62 | 元 |
| 充电总量 | 5,793,898.82 | kWh |
| 放电总量 | 4,699,570.62 | kWh |
| 未利用供能 | 5,086,721.14 | kWh |
| 2月1日0时库存 | 8,481.29 | kWh |
| 12月31日24时库存 | 1,245.09 | kWh |

正式期171天发生紧急购电，163天无紧急购电；共1859个10分钟区间发生紧急补购。发生紧急购电并不意味着负载未被满足，式(19)已补齐剩余缺口。不能把“无紧急购电天数比例”称为供电可靠率。

全年1—12月实际费用为18,251,147.68元，包含1月冷启动和预热；不能与其他策略2—12月费用直接排名。此次全年运行记录约69.67秒，其中日前LP约17.92秒、参数校准约50.94秒；该时间不含全部文件读取、Excel导出和论文图表生成，也不是其他硬件上的保证。

### 8.2 独立连续运行的策略对照

四种策略均从1月1日6000 kWh连续运行。分位数β=1、β=0和LDR使用相同的风险曲线生成方法；LDR日内库存不同，后续日期的计划随之变化。情景价值方案使用另一套日前情景规划，因此只作为整套策略比较。

| 方案 | 计划费／元 | 紧急费／元 | 实际总费／元 | 紧急电量／kWh |
|---|---|---|---|---|
| 分位数＋β=1 | 15,061,914.90 | 960,635.79 | 16,022,550.69 | 204,701.75 |
| 分位数＋β=0 | 15,066,766.13 | 849,168.20 | 15,915,934.33 | 140,430.52 |
| 情景价值控制 | 14,874,382.03 | 993,997.58 | 15,868,379.61 | 173,027.42 |
| 分位数＋LDR | 15,066,447.00 | 752,621.62 | 15,819,068.62 | 146,441.68 |

相对β=1，LDR总费减少203,482.07元，约1.2700%；相对β=0减少96,865.72元，约0.6086%；相对情景价值控制减少49,310.99元，约0.3108%。分位数三方案的正式期期初库存同为8481.289084 kWh，年末同为1245.090720 kWh，费用比较不依靠更少的期末存量。情景方案期初库存为9840.698695 kWh、年末为1237.849002 kWh，其边界差异随结果披露，不虚构库存退款。

![图1 各策略正式期费用比较](<figures/figure1_cost_comparison.png>)

图1数据来自各策略正式期汇总，柱形从零开始；总费差异相对总规模较小，紧急费分图进一步显示费用构成变化。结果支持在当前已比较策略中选择LDR为问题二主方案，不构成对所有可行预测和控制策略的最优性证明。

### 8.3 为什么紧急购电量略高，费用仍可更低

LDR相对β=0的紧急购电量增加6011.16 kWh，但紧急费减少96,546.58元；计划费同时减少319.13元，合计节费96,865.72元。紧急电量与紧急费用的关系为
...
|---|---|---|
| 分位数＋β=1 | 204,701.75 | 4.6929 |
| 分位数＋β=0 | 140,430.52 | 6.0469 |
| 情景价值控制 | 173,027.42 | 5.7447 |
| 分位数＋LDR | 146,441.68 | 5.1394 |
...
|---|---|---|---|
| LDR | 15,066,447.00 | 752,621.62 | 15,819,068.62 |
| β=1 | 15,066,447.00 | 960,690.53 | 16,027,137.52 |
| β=0 | 15,066,447.00 | 849,168.20 | 15,915,615.20 |

正式期LDR相对两种固定规则的配对累计费用分别减少208,068.91元和96,546.58元。相对β=0，LDR在59天费用更低、36天更高，其余239天持平。因此，不能宣称每天都改善；总费改善来自不同日期收益与损失的合计。配对试验逐日对齐库存，其累计结果不是三种策略各自连续运行的年度账单，也不允许按真实当天结果事后选择较优规则。

![图2 月度费用与配对节费](<figures/figure2_monthly_and_paired.png>)

图2上图为LDR真实月度账单，下图为同计划配对试验按月累计的差额，二者统计含义不同。正式期各月的配对累计差额为正，但这不等价于各日都为正。6月1日单日紧急费为77,611.90元，为正式期最高，说明少数不利日期仍会显著影响风险成本。

### 8.5 与完美信息下界的比较

将正式期全部实际供需视为预先已知，并匹配LDR的2月1日库存8481.289084 kWh，可使用此前已验证的匹配完美信息下界12,249,473.30元。该下界允许计划与储能全时域联合优化，且年末仅保留1200 kWh；实际LDR年末库存略高。

LDR实际费用高于该下界3,569,595.32元，约29.14%。差额混合了信息不确定性、风险修正、分层近似、有限搜索和边界处理等影响，不能全部归因为预测误差，不能称为不可约成本，也不能认为更换控制器就可以全部消除。该下界只作为性能参考，不是可执行策略。
...
|---|---|---|---|---|---|
| 2025-03-20 | 67,783.755175 | 40,791.66 | 162.160921 | 1,076.59 | 41,868.25 |
| 2025-06-21 | 74,778.282395 | 48,514.90 | 0.000000 | 0.00 | 48,514.90 |
| 2025-09-23 | 68,033.519435 | 42,102.59 | 680.668288 | 3,740.13 | 45,842.72 |
| 2025-12-21 | 90,827.497538 | 59,652.57 | 81.674650 | 177.54 | 59,830.10 |
...
|---|---|---|---|---|---|
| 10:00-10:10 | 0.000000 | 12:00-12:10 | 642.040355 | 14:00-14:10 | 0.000000 |
| 16:00-16:10 | 510.218474 | 18:00-18:10 | 701.674336 | 20:00-20:10 | 0.000000 |
| 全天购电量 | 67,783.755175 | 全天计划购电费 | 40,791.66 | 实际总费 | 41,868.25 |

**表：2025-03-20 实际储能充放电与边界库存（对应题目表2）**

| 时间段 | 充电量 | 放电量 | 时间段 | 充电量 | 放电量 |
|---|---|---|---|---|---|
| 0:00-4:00 | 8,451.746266 | 109.314576 | 4:00-8:00 | 1,821.903517 | 5,502.357037 |
| 8:00-12:00 | 5,611.144636 | 2,777.377217 | 12:00-16:00 | 5,067.650143 | 547.075114 |
| 16:00-20:00 | 161.088907 | 5,147.348878 | 20:00-24:00 | 510.574702 | 3,564.361750 |
| 0:00储电量 | 1,747.706317 | — | 24:00储电量 | 1,600.698590 | — |
...
| 时间段 | 购电量 | 时间段 | 购电量 | 时间段／指标 | 购电量／费用 |
|---|---|---|---|---|---|
| 10:00-10:10 | 0.000000 | 12:00-12:10 | 859.261410 | 14:00-14:10 | 0.000000 |
| 16:00-16:10 | 654.574650 | 18:00-18:10 | 738.848590 | 20:00-20:10 | 89.879029 |
| 全天购电量 | 74,778.282395 | 全天计划购电费 | 48,514.90 | 实际总费 | 48,514.90 |

**表：2025-06-21 实际储能充放电与边界库存（对应题目表2）**

| 时间段 | 充电量 | 放电量 | 时间段 | 充电量 | 放电量 |
|---|---|---|---|---|---|
| 0:00-4:00 | 1,961.278469 | 0.000000 | 4:00-8:00 | 1,167.285621 | 1,074.429050 |
| 8:00-12:00 | 159.169996 | 0.000000 | 12:00-16:00 | 0.000000 | 0.000000 |
| 16:00-20:00 | 0.000000 | 3,082.141560 | 20:00-24:00 | 4,614.939350 | 2,077.741214 |
| 0:00储电量 | 9,034.849378 | — | 24:00储电量 | 9,220.242333 | — |
...
| 时间段 | 购电量 | 时间段 | 购电量 | 时间段／指标 | 购电量／费用 |
|---|---|---|---|---|---|
| 10:00-10:10 | 0.000000 | 12:00-12:10 | 510.689314 | 14:00-14:10 | 0.000000 |
| 16:00-16:10 | 540.222690 | 18:00-18:10 | 775.671167 | 20:00-20:10 | 0.000000 |
| 全天购电量 | 68,033.519435 | 全天计划购电费 | 42,102.59 | 实际总费 | 45,842.72 |

**表：2025-09-23 实际储能充放电与边界库存（对应题目表2）**

| 时间段 | 充电量 | 放电量 | 时间段 | 充电量 | 放电量 |
|---|---|---|---|---|---|
| 0:00-4:00 | 9,119.694448 | 24.505000 | 4:00-8:00 | 1,551.314281 | 6,292.100285 |
| 8:00-12:00 | 5,098.874106 | 1,896.615717 | 12:00-16:00 | 5,409.042448 | 506.239128 |
| 16:00-20:00 | 95.742869 | 4,986.797268 | 20:00-24:00 | 385.103888 | 3,666.628288 |
| 0:00储电量 | 1,267.216821 | — | 24:00储电量 | 1,457.805341 | — |
...
| 时间段 | 购电量 | 时间段 | 购电量 | 时间段／指标 | 购电量／费用 |
|---|---|---|---|---|---|
| 10:00-10:10 | 0.000000 | 12:00-12:10 | 978.149079 | 14:00-14:10 | 0.000000 |
| 16:00-16:10 | 848.622962 | 18:00-18:10 | 713.879457 | 20:00-20:10 | 0.000000 |
| 全天购电量 | 90,827.497538 | 全天计划购电费 | 59,652.57 | 实际总费 | 59,830.10 |

**表：2025-12-21 实际储能充放电与边界库存（对应题目表2）**

| 时间段 | 充电量 | 放电量 | 时间段 | 充电量 | 放电量 |
|---|---|---|---|---|---|
| 0:00-4:00 | 2,785.365448 | 0.000000 | 4:00-8:00 | 953.014482 | 661.046495 |
| 8:00-12:00 | 5,031.143019 | 7,948.909940 | 12:00-16:00 | 7,756.197983 | 2,408.836271 |
| 16:00-20:00 | 0.000000 | 5,031.622167 | 20:00-24:00 | 441.526921 | 3,577.277452 |
| 0:00储电量 | 8,169.954169 | — | 24:00储电量 | 1,631.930208 | — |
...
|---|---|---|---|---|---|---|---|
| 17:00-17:20 | 19.132572 | 无 | 0.000000 | 14:40-15:10 | 230.934277 | 01:10-01:20 | 12.916833 |
| 20:40-20:50 | 138.627494 | — | — | 15:20-15:30 | 5.445476 | 03:30-03:50 | 68.757817 |
| 21:20-21:30 | 4.400855 | — | — | 16:10-16:50 | 112.337921 | — | — |
| — | — | — | — | 17:00-17:20 | 9.046832 | — | — |
| — | — | — | — | 20:10-20:20 | 10.234367 | — | — |
| — | — | — | — | 20:40-21:10 | 312.669414 | — | — |

6月21日无紧急购电。该日未利用供能达到53,775.18 kWh，表明较低短缺风险仍可能伴随富余电量不能利用，不能仅凭零紧急购电判定其经济性达到最优。

### 9.7 日内运行特征

![图3 指定日期日内运行](<figures/figure3_key_dates.png>)

3月20日和9月23日均出现中午实际净负荷为负的时段，光伏可覆盖负载并形成充电或未利用供能；晚间仍存在短时补购。6月21日净负荷中午下降更明显、库存长时间较高，无紧急购电。12月21日图中净负荷保持为正，储能主要通过购电与需求的时序配合进行调节，其紧急购电仅发生在凌晨两个合并区间。上述描述针对图示四天，不直接外推为全年季节规律。
...
对全年52560个区间重新检查供需平衡、内部库存递推、跨日衔接、容量功率界、充放电互斥以及紧急充电禁用条件。正式期费用从原电价独立计算，结果与汇总一致。

| 核验项目 | 结果 |
|---|---|
| 最大供需平衡误差 | 7.11e-13 kWh |
...
| 同时充放电区间 | 0 |
| 紧急购电充电区间 | 0 |
| 独立复算正式期总费 | 15,819,068.62元 |
| 核验结论 | 通过 |

### 10.2 数据、计划、规则与情景评分复核

本次论文整理时，将保存的全年负载、光伏与附件2逐项对照，将电价与附件1对照。进一步使用各天保存的真实起始库存重新生成364天购电计划，最大差异约$3.02\times10^{-9}$ kWh；不含无需优化的1月1日零计划。

利用保存参数和原附件，独立逐时重建全部52560个动作、阈值与阶段信号，最大差异约$5.46\times10^{-12}$ kWh。对337个具备完整残差窗的日期重新计算已选参数与零参数情景评分，最大差异约$3.87\times10^{-12}$元，全部满足已实现的零参数保底条件。这些检查验证保存结果与模型、代码口径一致，不是证明有限搜索已达到全局最优。
...
LDR的结果文件保留官方三个工作表及表头。已回读334日计划购电、2004个四小时储能块及全部728条紧急购电表记录，并独立复算费用；表内数值与调度明细最大误差约$2.27\times10^{-13}$ kWh。728条是包含“无紧急购电”记录的表行数，不应称为728次紧急事件。模板的既定跨日映射见附录B；回读通过证明文件符合该映射和明细，并不证明模板时间标签只有唯一解释。
...
**模型局限。**第一，七天均值和21日残差可能不能充分表达突变和长期趋势，经验情景的代表性有限。第二，第一层风险曲线是逐时分位数，不能完整保留整日联合概率结构。第三，单日终端价值只是一阶近似，没有精确解决跨日价值传递。第四，四阶段误差信号压缩了历史信息，其充分性未经证明。第五，差分进化在有限预算下仅给出近似候选，缺少全局最优证书；本次只保底β=1，不保底β=0。第六，目前没有单独移除仿射斜率的对照，不能把全部改善归于$\lambda$，截距调整与反馈共同构成已评价的控制器。第七，尚未提供多随机种子、参数范围、残差窗口和终端价值的系统稳健性试验；不把已有基线比较称为充分敏感性分析。

所有逐日决策按此前可用信息生成，但在同一年度比较不同方法后选定主方案，仍属于本年度样本上的模型选择，不等于独立外部年度验证。后续可用额外年份或严格嵌套历史验证评估泛化表现。论文保留这些限制，不影响当前结果作为问题二可实施策略解答。

## 12 问题二结论

问题二最终采用80%分位数风险修正与截断仿射库存保留规则的分层方案：日前用历史预测及误差情景生成普通购电计划，固定该计划后校准七维保留规则，日内依据当前供需与阶段已观测误差执行同一充放电函数。全年库存连续，所有实际缺口均由储能或紧急购电补足。

正式期实际总费为**15,819,068.62元**，相对β=1和β=0分位数基线分别降低约**1.27%**和**0.61%**。四个指定日期及完整策略均已输出并通过独立核验。当前回测结果支持将LDR作为主方案，并将固定保留、即时补缺与情景价值方法作为对照；结论限于本模型假设、既定数据与当前已验证算法配置。
...
- [月度汇总与论文数据表](<tables>)
- [匹配完美预见下界](<../../benchmark/perfect_foresight/完美预见下限测算说明.md>)

复现顺序：先运行LDR主程序生成独立结果目录，再按已有工作簿导出流程回填模板，之后进行工作簿与LDR专项校验。本稿的补充复核与图表读取既有账本，不修改策略参数或购电计划。
...
第二条命令前应完成已有工作簿导出步骤；直接运行主程序不应被解释为已经自动完成Excel导出与回读。当前result2.xlsx已经存在且回读通过。
...
因此，“全天购电量”使用当天144个物理区间的真实合计，不能简单以本行144个展示时段列求和；这也意味着2月1日首区间不在2月1日本行时段列中，完整当天计划应结合逐时CSV读取。该非标准映射保持既有交付与原表头一致，属于应随结果说明的口径，未在本次论文整理中重新定义。

“全天购电费”列填计划购电费，紧急购电另表列示并在式(23)中合并为实际总费。论文表1型表保留计划费字段，另以日期汇总表明确实际总费用。充放电量表填实际执行而非日前参考动作；0时和24时库存为物理日边界状态。

## 附录C 数据依据与图表说明

原始依据为赛题问题二、附录1储能参数、附件1电价、附件2实际供需及附件5模板。正文结果来自已核验的LDR调度明细、逐日参数和独立基线汇总；完美信息下界来自已保存的同起始库存、同正式期模型。所有费用改进比例按“对照费用−LDR费用”除以对照费用计算。
...
|---|---|---|---|---|
| 2025-03-20 | -2817.8084 | -4649.1348 | -354.2429 | -7817.7547 |
| 2025-06-21 | 8829.2802 | -8757.5903 | -245.6564 | -343.5318 |
| 2025-09-23 | -4727.2538 | -4831.3819 | -249.0490 | -557.6235 |
| 2025-12-21 | 3495.9368 | -592.8710 | -505.5429 | -1956.8989 |

| 日期 | λ2 | λ3 | λ4 | 相对零参数经验目标改善／元 |
|---|---|---|---|---|
| 2025-03-20 | 1.300197 | 0.999772 | 1.951428 | 457.16 |
| 2025-06-21 | -1.080807 | 1.699471 | 1.676757 | 194.25 |
| 2025-09-23 | 0.612704 | 0.351951 | -0.704694 | 592.52 |
| 2025-12-21 | 0.860172 | 1.503817 | -1.164375 | 197.57 |

该经验目标包含终端库存价值代理，并在历史情景上平均，不能当作真实当天节费。例如6月21日虽存在正的历史情景目标改善，但真实当天LDR和原β=1均无紧急购电；历史情景优化与实际日收益不要求一一对应。截距或斜率单独的符号不能直接解释为当日全部时段更保守，因为参考轨迹、误差信号和截断会共同决定最终阈值。

...
> 本目录集中存放问题二的全部结果、分析与历史对照。**当前正式答案 = LDR（负载周持久化＋光伏七天均值）**（`current/ldr/result2.xlsx`，2—12 月实际总购电费 13,978,077.23 元）。该方案在当前数据口径、参数边界、搜索预算和已完成对照范围内费用最低；相似日高斯核暂不接入本轮费用回测，因此不在该排序范围内。

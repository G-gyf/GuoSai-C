...
// Question 4-2 result4-2.xlsx backfill (fluctuating prices, question-2 pipeline).
...
const wb = await SpreadsheetFile.importXlsx(await FileBlob.load(path.join(root, '附件/附件5/result4-2.xlsx')));
...
// 计划购电量: 143 same-day template slots + next-day first slot + day total + planned bill
...
await result.save(path.join(out, 'result4-2.xlsx'));
console.log(JSON.stringify({ output: 'result4-2.xlsx', days: p.purchases.length, batteryRows: p.batteries.length, emergencyRows: p.emergency.length }));
process.exit(0);

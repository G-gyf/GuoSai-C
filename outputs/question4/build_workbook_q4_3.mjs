...
// Question 4-3 result4-3.xlsx backfill (fluctuating prices, question-3 pipeline).
...
const wb = await SpreadsheetFile.importXlsx(await FileBlob.load(path.join(root, '附件/附件5/result4-3.xlsx')));
...
await result.save(path.join(out, 'result4-3.xlsx'));
console.log(JSON.stringify({ output: 'result4-3.xlsx', days: p.purchases.length, batteryRows: p.batteries.length, emergencyRows: p.emergency.length }));
process.exit(0);
